...
const Header = () => {
  ...
  const options: IOption[] = [
    ...
    },
    {
      ...
      options: [
        {
          label: "Abertura de Manifestação Anônimo",
          href: "/"
        },
        {
          label: "Abertura de Manifestação Identificado",
          href: "/"
        },
        {
    ...
    >
      <section className="container flex items-center justify-between m-auto p-4 h-20 relative">
        ...
        <ul
          className={
            clsx(
              "flex flex-col gap-4 justify-center items-center h-[calc(100dvh-80px)] md:justify-center h-[calc(100vh - 80px)] absolute transition-all duration-500 translate-y-0 bottom-0 left-0 bg-white-50 w-full text-black text-2xl z-10",
        ...
        >
          {options.map((option) => (
            <li key={option.label} onMouseEnter={() => handleMouseEnter(option)}>
              <Link href={option?.href || "#"} className={clsx("flex gap-2 md:text-xs md:font-light xl:text-base hover:font-bold items-center text-primary-500 cursor-pointer", selectedOption?.label === option.label && "font-bold")}>
                {option.label}
                {option?.options.length > 0 && <svg width="13" height="8" viewBox="0 0 13 8" fill="none" xmlns="http://www.w3.org/2000/svg">
