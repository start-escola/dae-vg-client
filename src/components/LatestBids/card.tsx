...
const LatestBidsCard = ({ title, opening, closing, status, slug, last_status }: ILatestBidsCard) => {
  ...
  return (
    <li className="m-auto flex flex-col justify-between text-primary-500 px-6 pt-5 pb-4 border-b-4 shadow-xl w-fit">
      <div className="flex flex-col gap-4">
        <div>
          <Link href={`/transparencia/licitacoes/${opening?.split("/")[0]}/${slug}`} className="flex flex-col items-center text-center">
        ...
        <ul className="min-h-32">
          ...
          {
            showAllStatus && status.map(({ name, date }) => (
              <li key={name}><strong>{name}:</strong> {date?.replaceAll('-', '/')}</li>
            ))
          }
          {
            last_status && <li><strong>{last_status.name}:</strong> {last_status.date?.replaceAll('-', '/')}</li>
          }
        </ul>
      </div>
      <button className={`justify-self-end py-1 text-white-0 font-semibold text-base rounded bg-primary-500 ${last_status?.name === "Finalizada" && "opacity-40"}`} aria-label="Bid Status">
